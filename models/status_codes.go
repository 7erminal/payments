...
func GetStatus_codesById(id int64) (v *Status_codes, err error) {
	o := orm.NewOrm()
	v = &Status_codes{StatusCodeId: id}
	if err = o.QueryTable(new(Status_codes)).Filter("StatusCodeId", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func UpdateStatus_codesById(m *Status_codes) (err error) {
	o := orm.NewOrm()
	v := Status_codes{StatusCodeId: m.StatusCodeId}
...
func DeleteStatus_codes(id int64) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Status_codes{StatusCodeId: id}); err == nil {
